...
class ZajemTock(Node):
    def __init__(self):       
        super().__init__('zajem_tock')      
        self.get_logger().info("ZajemTock node initialized.")


        #ustavarili bomo server
        self.srv = self.create_service(SetBool, 'zajemi_sliko', self.zajemi_sliko_callback)
        ...
        mesh = o3d.io.read_triangle_mesh("/home/lab-3d/dobot_ws/src/DOBOT_6Axis_ROS2_V4/kamera/mesh/mesh_from_model_aligned.ply")
        #mesh = o3d.geometry.simpliftimey_vertex_clustering(mesh, voxel_size=0.1)
        mesh.scale(0.3, center=mesh.get_center()) # Scale the mesh down to 30% of its original size

        mesh.compute_vertex_normals()#izračuna normale


        # corect mesh self-intersections
        ...
        o3d.io.write_triangle_mesh(f"/home/lab-3d/realsense_point_cloud_0.ply", mesh)
        ...
        # keep only triangels that have normal in y direction grater than normal_orientation_threshold
        ...
        meshyp = self.remove_small_clusters(meshyp, min_cluster_size_ratio=min_cluster_size_ratio)
        meshxp = self.remove_small_clusters(meshxp, min_cluster_size_ratio=min_cluster_size_ratio)
        meshym = self.remove_small_clusters(meshym, min_cluster_size_ratio=min_cluster_size_ratio)
        meshxm = self.remove_small_clusters(meshxm, min_cluster_size_ratio=min_cluster_size_ratio)
        ...
        scanning_points += self.get_all_scanning_points(mesh_work=meshyp, mesh_test_org=mesh)
        scanning_points += self.get_all_scanning_points(mesh_work=meshxp, mesh_test_org=mesh)
        scanning_points += self.get_all_scanning_points(mesh_work=meshym, mesh_test_org=mesh)
        scanning_points += self.get_all_scanning_points(mesh_work=meshxm, mesh_test_org=mesh)
    ...
    def zajemi_sliko_callback(self, requuest, response):

        response.data = True
        return response
        

    def remove_small_clusters(self, mesh, min_cluster_size_ratio=None):
        """    
...
def main(args=None):
    ...
    zajem_tock = ZajemTock()
    rclpy.spin(zajem_tock)
    zajem_tock.destroy_node()
    rclpy.shutdown()
